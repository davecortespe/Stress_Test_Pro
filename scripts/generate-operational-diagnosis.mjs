...
function inferMissingFieldsFromAssumptions(assumptions) {
...
function getOutgoingStepIds(model, stepId) {
...
export function buildOperationalDiagnosis(model, output, scenario) {
  ...
  const bottleneckEntry =
    ...
    nodeMetrics.slice().sort((a, b) => (b[1].bottleneckIndex ?? -1) - (a[1].bottleneckIndex ?? -1))[0] ??
    null;

  const bottleneckId =
    bottleneckEntry?.[0] ?? model.baseline?.bottleneckStepId ?? model.stepModels?.[0]?.stepId ?? "";
  ...
  const downstreamEntry =
    downstreamIds
      .map((stepId) => [stepId, output.nodeMetrics?.[stepId]] )
...
export function toMarkdown(diagnosis) {
  return [
...
if (isEntrypoint) {
  ...
  if (!args.model || !args.scenario || !args.metrics) {
    throw new Error(
      "Usage: node scripts/generate-operational-diagnosis.mjs --model <compiled_forecast_model.json> --scenario <scenario_committed.json> --metrics <result_metrics.json> [--outJson <file>] [--outMd <file>]"
    );
  }

  const model = readJson(path.resolve(String(args.model)));
  const scenario = readJson(path.resolve(String(args.scenario)));
  const metrics = readJson(path.resolve(String(args.metrics)));
  const diagnosis = buildOperationalDiagnosis(model, metrics, scenario);
  const markdown = toMarkdown(diagnosis);

  if (args.outJson) {
    writeJson(path.resolve(String(args.outJson)), diagnosis);
  }
  if (args.outMd) {
    fs.writeFileSync(path.resolve(String(args.outMd)), `${markdown}\n`, "utf8");
  }
  if (!args.outJson && !args.outMd) {
    process.stdout.write(`${JSON.stringify({ diagnosis, markdown }, null, 2)}\n`);
  }
}
