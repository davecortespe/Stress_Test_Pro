...
export function bindParameterGroupsToForecast(groups, forecastModel) {
    ...
    return groups.map((group) => ({
        ...
        fields: group.fields.map((field) => {
            ...
            return {
                ...
                defaultValue: forecastModel.inputDefaults[field.key] ?? input.defaultValue ?? field.defaultValue
            };
        })
    }));
}
export function getDefaultScenario(groups, inputDefaults) {
    const defaults = {};
...
export function scenarioEquals(a, b) {
...
export function stepScenarioKey(stepId, field) {
...
export function buildInspectorValues(step, scenario, defaults) {
    ...
    return {
        capacityUnits: Math.max(1, Math.round(toNumber(scenario[stepScenarioKey(step.stepId, "capacityUnits")], Math.max(1, Math.round(step.effectiveUnits))))),
        ctBaseline: Math.max(0.01, toNumber(scenario[stepScenarioKey(step.stepId, "ctBaseline")], step.ctMinutes ?? 1)),
