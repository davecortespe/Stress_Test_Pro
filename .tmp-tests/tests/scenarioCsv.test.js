...
import { buildScenarioLibraryColumns, createScenarioLibraryContext, mergeScenarioRowWithBaseline, parseScenarioLibrary, serializeScenarioLibrary, upsertScenarioLibraryEntry } from "../src/lib/scenarioCsv.js";
...
run("serialize and parse scenario library round-trip mixed values", () => {
    ...
    const csv = serializeScenarioLibrary(entries, createScenarioLibraryContext("Ops App", "Model A"), scenarioColumns);
...
run("mergeScenarioRowWithBaseline overlays only defined values", () => {
    assert.deepEqual(mergeScenarioRowWithBaseline({
