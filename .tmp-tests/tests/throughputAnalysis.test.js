...
import { createScenarioLibraryContext, parseScenarioLibrary, serializeScenarioLibrary } from "../src/lib/scenarioCsv.js";
...
run("capacity unit increases add FTE cost and survive save-load round-trip", () => {
    ...
    ], createScenarioLibraryContext("Ops App", "Model A"), ["step_cut_capacityUnits", "sellingPricePerUnit"]);
...
run("CSV builders include the expected summary and step fields", () => {
    ...
    assert.match(stepCsv, /Step Name,Material Cost Per Unit,Labor Rate Per Hour,Added FTEs,Added FTE Labor Per Unit,Labor Cost Per Unit,Equipment Rate Per Hour,Equipment Cost Per Unit,Total Step Cost Per Unit,Missing Costs/);
