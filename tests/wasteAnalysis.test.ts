...
} from "../src/lib/wasteAnalysis.ts";
import type { CompiledForecastModel, SimulationOutput } from "../src/types/contracts.ts";
...
}): CompiledForecastModel["stepModels"][number] {
  return {
    stepId: args.stepId,
...
function createModel(args: {
  ...
  edges: Array<{ from: string; to: string; probability?: number }>;
  startNodes: string[];
  endNodes: string[];
  stepModels: CompiledForecastModel["stepModels"];
...
function createOutput(
  ...
  leadTimesByStepId: Record<string, number | null | undefined>
): SimulationOutput {
...
run("parallel branch totals are weighted by routed demand instead of naive summation", () => {
  ...
  const analysis = buildWasteAnalysis(
    model,
    model.inputDefaults,
    createOutput(model, { entry: 5, branch_a: 30, branch_b: 80 })
  );
...
run("scenario LT overrides change waste without changing throughput, and CT multipliers change the benchmark", () => {
  ...
  assertClose(
    Number(baseOutput.globalMetrics.forecastThroughput),
    Number(ltOutput.globalMetrics.forecastThroughput)
  );
