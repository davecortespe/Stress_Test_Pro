...
} from "../src/lib/throughputAnalysis.ts";
import type { CompiledForecastModel, MasterData } from "../src/types/contracts.ts";

function assertClose(actual: number | null, expected: number, epsilon = 1e-9): void {
  assert.ok(typeof actual === "number", `Expected numeric value close to ${expected}, received ${actual}`);
  assert.ok(Math.abs(actual - expected) <= epsilon, `Expected ${actual} to be within ${epsilon} of ${expected}`);
}

function createModel(): CompiledForecastModel {
  return {
    version: "test",
...
run("buildThroughputAnalysis computes costs, leverage, and low-efficiency status", () => {
  ...
  assertClose(
    analysis.summary.fullyLoadedProfitPerUnit,
    100 - 20 - expectedLaborCostPerUnit - expectedEquipmentCostPerUnit
  );
  ...
  assertClose(
    analysis.profitLossRows[0]?.total ?? null,
    (analysis.summary.sellingPrice ?? 0) * completedUnits
  );
  assertClose(
    analysis.profitLossRows[4]?.total ?? null,
    (analysis.summary.fullyLoadedProfitPerUnit ?? 0) * completedUnits
  );
  assertClose(
    analysis.stepRows.reduce((sum, row) => sum + (row.totalStepCost ?? 0), 0),
    (analysis.summary.materialCostPerUnit ?? 0) +
